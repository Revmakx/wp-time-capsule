...
function ask_backup_name_wptc(type){
	...
	if (type === 'bbu') {
		var data = {
			bbu_note_view:{
				type: 'message',note:'We will notify you once the backup and updates are completed. <br> <a href="https://docs.examplewptc.com/article/update-plugins-without-backup" target="blank">  Click here</a> if you want to update immediately without backup.',
			},
		};
	...
	jQuery.post(ajaxurl, {
		...
		if (data.success) {
			...
			swal({
				...
				html               : wptc_get_dialog_body('Issue sending failed, try after sometime or email us at <a href="mailto:help@examplewptc.com?Subject=Contact" target="_top">help@examplewptc.com</a>.', 'error'),
				padding            : '0px 0px 10px 0',
				buttonsStyling     : false,
				confirmButtonColor : '',
				confirmButtonClass : 'button-primary wtpc-button-primary',
...
function show_users_backend_errors_wptc(error){
	...
	if (error) {
		var note = 'WP Time Capsule Error : '+ error + '   <br>If you are not sure what went wrong, please email us at <a href="mailto:help@examplewptc.com?Subject=Contact" target="_top">help@examplewptc.com</a>';
