...
function get_staging_current_status_key_wptc(status, statusText){
		...
		try{
			...
			jQuery("#staging_progress_bar_note").html('Unknown error, email us at <a href="mailto:help@examplewptc.com?Subject=Contact" target="_top">help@examplewptc.com</a> ').css('color', '#ca4a1f');
		}
	}). fail(function(request) {
		jQuery("#staging_progress_bar_note").html('Unknown error, email us at <a href="mailto:help@examplewptc.com?Subject=Contact" target="_top">help@examplewptc.com</a> ').css('color', '#ca4a1f');
	});
...
function wptc_staging_completed_template(){
	...
	var push_to_live =	'<div style="margin: 0px 0px 10px 0px;"> <a href="http://docs.examplewptc.com/article/28-how-push-staging-site-to-production" target="_blank"> How do I push my staging changes to the live site? </a></div>';
