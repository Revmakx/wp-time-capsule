...
class Wptc_InitialSetup extends Wptc_InitialSetup_Init {
	...
	public function get_select_cloud_dialog_div() {
		...
		if (!empty($sites_count) && $sites_count >= WPTC_GDRIVE_TOKEN_ON_INIT_LIMIT) {
			...
			$div .= '<a href="http://docs.examplewptc.com/article/23-add-new-site-using-existing-google-drive-token" id="see_how_to_add_refresh_token_wptc" target="_blank" style="text-decoration: none;position: absolute;cursor: pointer;top: 54px;right: 349px;font-size: 12px;">Need help ?</a></div>';
		}
		if (is_php_version_compatible_for_s3_wptc()) {
