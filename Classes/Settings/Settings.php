...
class Wptc_Settings extends Wptc_Settings_Init {
	...
	public function update_footer( $update_footer_text ) {
		...
		if ( isset( $_REQUEST[ 'page' ] ) && strpos($_REQUEST[ 'page' ], 'wp-time-capsule' ) !== false ) {
			$update_footer_text  = '<span class="wp-time-capsule-update-footer"><a href="' . __( 'https://examplewptc.com', 'wp-time-capsule' ) . '">' . $this->get_plugin_data( 'Name' ) . '</a> '. sprintf( __( 'version %s' ,'wp-time-capsule'), $this->get_plugin_data( 'Version' ) ) . '</span>';
	...
	public function save_settings_revision_limit($requested_revision_limit){
		...
		if ($default_repo === 'dropbox') {
			...
			if ($modified_revision_limits < $requested_revision_limit) {
				$help_doc = '<a href="http://docs.examplewptc.com/article/30-how-to-enable-120-days-restore-points" target="_blank">Know more ?</a>';
